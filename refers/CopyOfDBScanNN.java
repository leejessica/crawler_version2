...
public class CopyOfDBScanNN {
	...
	public CopyOfDBScanNN(Vector[] grids, double epsi, int min) {
		this.grids = grids;
		this.states = new int[grids.length];

		for (int i = 0; i < this.grids.length; i++) {
			this.states[i] = CopyOfDBScanNN.unvisited;// set every point to be unsupervised
	...
	public CopyOfDBScanNN(Vector[] pois, double epsi) {
		this.grids = pois;
		this.states = new int[grids.length];
	...
	public void clustering() {
		...
		for (int i = 0; i < this.grids.length; i++) {
			...
			if (this.states[i] == CopyOfDBScanNN.unvisited) {
				this.states[i] = CopyOfDBScanNN.visited;

				neighbors = findNeighbors(i);

				if (neighbors.size() < this.minPts) {
					this.states[i] = CopyOfDBScanNN.noisy;
	...
	public void expandClusterByRatio(int pointID, TreeSet neighbors, DBScanNNCluster cluster) {
		...
		while (!neighbors.isEmpty()) {
			...
			if (this.states[tmpID] == CopyOfDBScanNN.unvisited) {
				this.states[tmpID] = CopyOfDBScanNN.visited;
	...
	public RSTree getrTree() {
		return rTree;
	}

	public void setrTree(RSTree rTree) {
		this.rTree = rTree;
	}
	...
	public int getNumNoisyPoints() {
		...
		for (int i = 0; i < this.states.length; i++) {
			if (this.states[i] == CopyOfDBScanNN.noisy) {
				count++;
			}
		}
